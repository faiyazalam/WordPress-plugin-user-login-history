...
class Reader extends AbstractCsv implements TabularDataReader, JsonSerializable {
	...
	protected array $formatters = array();
	...
	protected function setHeader( int $offset ): array {
		...
		return match ( true ) {
			...
			array( null ) === $header,
			array( false ) === $header,
			array( '' ) === $header && 0 === $offset && '' !== $inputBom => throw SyntaxError::dueToHeaderNotFound( $offset ),
			default => $header,
		};
	}
	...
	protected function removeBOM( array $record, int $bom_length, string $enclosure ): array {
		if ( array() === $record || ! is_string( $record[0] ) || 0 === $bom_length || strlen( $record[0] ) < $bom_length ) {
			return $record;
		}
	 ...
	 * @throws Exception
	 */
	public function nthAsObject( int $nth, string $className, array $header = array() ): ?object {
	 ...
	 * @throws Exception
	 */
	public function firstAsObject( string $className, array $header = array() ): ?object {
	...
	public function jsonSerialize(): array {
		return array_values( array( ...$this->getRecords() ) );
	}
	 ...
	 */
	public function getRecords( array $header = array() ): Iterator {
	 ...
	 * @return iterator<T>
	 */
	public function getRecordsAsObject( string $className, array $header = array() ): Iterator {
	...
	protected function prepareRecords(): Iterator {
		$normalized = fn ( $record ): bool => is_array( $record ) && ( $this->is_empty_records_included || $record !== array( null ) );
		$bom        = '';
		...
		if ( $this->is_empty_records_included ) {
			$records = new MapIterator( $records, fn ( array $record ): array => ( array( null ) === $record ) ? array() : $record );
		}
	...
	protected function stripBOM( Iterator $iterator, string $bom ): Iterator {
		...
		return new CallbackFilterIterator(
			...
			fn ( array $record ): bool => $this->is_empty_records_included || $record !== array( null )
		);
	}
	 ...
	 */
	protected function prepareHeader( $header = array() ): array {
	...
	protected function computeHeader( array $header ): array {
		...
		return match ( true ) {
			...
			array() !== array_filter( array_keys( $header ), fn ( string|int $value ) => ! is_int( $value ) || $value < 0 ) => throw new SyntaxError( 'The header mapper indexes should only contain positive integer or 0.' ),
			default => $header,
		};
	}
	...
	protected function combineHeader( Iterator $iterator, array $header ): Iterator {
		...
		return match ( array() ) {
			...
			default => new MapIterator(
				$iterator,
				function ( array $record ) use ( $header, $formatter ): array {
					$assocRecord = array();
	 ...
	 * @throws TypeCastingFailed
	 */
	public function getObjects( string $className, array $header = array() ): Iterator {
