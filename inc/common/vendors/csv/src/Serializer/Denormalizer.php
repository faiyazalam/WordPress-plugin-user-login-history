...
final class Denormalizer {
	 ...
	 */
	public function __construct( string $className, array $propertyNames = array() ) {
	...
	public static function types(): array {
		$default = array( ...array_column( Type::cases(), 'value' ), ...CallbackCasting::types() );
	 ...
	 * @throws TypeCastingFailed
	 */
	public static function assignAll( string $className, iterable $records, array $propertyNames = array() ): Iterator {
	...
	private function setPropertySetters( array $propertyNames ): array {
		...
		foreach ( array( ...$this->properties, ...$this->class->getMethods() ) as $accessor ) {
	...
	private function autoDiscoverPropertySetter( ReflectionMethod|ReflectionProperty $accessor, array $propertyNames, array $methodNames ): ?PropertySetter {
		...
		if ( $accessor instanceof ReflectionMethod ) {
			...
			if ( array() === $accessor->getParameters() ) {
				return null;
			}
		...
		[$offset, $reflectionProperty] = match ( true ) {
			$accessor instanceof ReflectionMethod => array( array_search( $accessor->getName(), $methodNames, true ), $accessor->getParameters()[0] ),
			$accessor instanceof ReflectionProperty => array( array_search( $accessor->getName(), $propertyNames, true ), $accessor ),
		};
	...
	private function findPropertySetter( MapCell $cell, ReflectionMethod|ReflectionProperty $accessor, array $propertyNames ): ?PropertySetter {
		...
		return match ( true ) {
			...
			array() !== $propertyNames && $offset > count( $propertyNames ) - 1 => throw new MappingFailed( 'offset integer position can not exceed property names count.' ),
	...
	private function getMethodFirstArgument( ReflectionMethod $reflectionMethod ): ReflectionParameter {
		...
		return match ( true ) {
			array() === $arguments => throw new MappingFailed( 'The method `' . $reflectionMethod->getDeclaringClass()->getName() . '::' . $reflectionMethod->getName() . '` does not use parameters.' ),
	 ...
	 * @throws MappingFailed
	 */
	private function resolveTypeCasting( ReflectionProperty|ReflectionParameter $reflectionProperty, array $options = array() ): TypeCasting {
