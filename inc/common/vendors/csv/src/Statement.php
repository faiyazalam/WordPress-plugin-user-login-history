...
class Statement {
	 ...
	 * @throws SyntaxError
	 */
	public function process( TabularDataReader $tabular_data, array $header = array() ): TabularDataReader {
		if ( array() === $header ) {
	...
	protected function applySelect( Iterator $records, array $recordsHeader ): TabularDataReader {
		...
		$header  = array_reduce( $this->select, $selectColumn, array() );
		...
		return new ResultSet( $records, $hasHeader ? $header : array() );
	}
